...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    for p in self.legalPositions:
      ...
      # New beliefs calculated using the Bayes rule formula:
      #   P(trueDistance | noisyDistance) = 
      #         P(noisyDistance | trueDistance) * P(noisyDistance) / P(trueDistance)
  ...
  def elapseTime(self, gameState):
    ...
    aggregateNewBeliefs = util.Counter()
    # Iterate through all possible positions a ghost could occupy at time t
    for oldPos in self.legalPositions:
      # Get probability distribution based on this assumed 'old' position
      newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
      # For each possible new position, weight the probability of that movement against our beliefs about
      # how likely it is that the ghost occupied the old position, and add that to the new belief value
      for newPos in newPosDist:
        aggregateNewBeliefs[newPos] += newPosDist[newPos] * self.beliefs[oldPos]

    self.beliefs = aggregateNewBeliefs
...
class ParticleFilter(InferenceModule):
  ...
  def initializeUniformly(self, gameState, numParticles=300):
    ...
    self.particles = [random.choice(self.legalPositions) for _ in range(0, numParticles)]
  ...
  def observe(self, observation, gameState):
    ...
    for particle in self.particles:
      ...
      # If all weights are 0 (ghost is eaten), util.sample() will throw an error.
      # We therefore keep track of how many 0 weights there are so we can check for this
      if emissionModel[trueDistance] == 0:
        zeroWeightCount += 1

    if zeroWeightCount != self.numParticles:
      # Resample based on new weights
      for _ in range(self.numParticles):
        newParticleList.append(util.sample(particleWeights))
      self.particles = newParticleList
    else:
      # If the ghost is eaten, get rid of all its particles
      self.particles = []
      self.numParticles = 0
  ...
  def getBeliefDistribution(self):
    ...
    """
    # I copied this from the JointParticleFilter function further down the page.
