    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      if not self.walls[nextx][nexty]:
        ...
        for i, corner in enumerate(self.corners):
          if nextPos == corner and nextCor[i] is False:
            nextCor[i] = True
        successors.append( ( (nextPos, nextCor), action, cost) )      
    self._expanded += 1
    return successors
...
def cornersHeuristic(state, problem):
  ...
  distances = []
  unvisitedCorners = []
  ((stateX, stateY), visitedList) = state
  # Don't take into account already eaten pellets
  for i, isVisited in enumerate(visitedList):
    if not isVisited:
      unvisitedCorners.append(corners[i])
  # If all pellets are eaten, done
  if len(unvisitedCorners) < 1:
    return 0
  # Otherwise, return the distance to the furthest pellet
...
def foodHeuristic(state, problem):
  ...
  """
  isWall = problem.walls
  (posX, posY), foodGrid = state
  foodList = foodGrid.asList()
  # If all pellets are eaten, done
  if len(foodList) < 1:
    return 0
  # Otherwise, return the distance to the furthest pellet
  distances = []   # Distance to each remaining pellet from node
  for (foodX, foodY) in foodList:
    currDistance = abs(foodX - posX) + abs(foodY - posY)
